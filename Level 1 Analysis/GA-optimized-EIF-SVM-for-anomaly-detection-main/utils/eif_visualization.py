...
def plot_eif_anomaly_scores(results_df, threshold):
    ...
    plt.scatter(normal_df.index, normal_df['eif_score'],
                c='blue', label='Normal', alpha=0.5, s=10)

    plt.scatter(anomaly_df.index, anomaly_df['eif_score'],
                c='red', label='Anomaly', alpha=0.8, s=30)

    plt.axhline(threshold, color='black', linestyle='--', label=f'Threshold ({threshold:.4f})')

    plt.xlabel("Timestamp")
    plt.ylabel("EIF Anomaly Score")
    plt.title("EIF Anomaly Detection Results")
    plt.legend()
    plt.grid(True)
    plt.tight_layout()
    plt.show()
